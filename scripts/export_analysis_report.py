...
from analyze_abinit_dos import analyze_path as analyze_dos
from analyze_abinit_band import analyze_path as analyze_band
...
def render_markdown(record: dict[str, object]) -> str:
    ...
    if record.get("final_etotal_Ha") is not None:
        lines.append(f"- Final energy (Ha): `{record['final_etotal_Ha']:.8f}`")
    if record.get("energy_per_atom_Ha") is not None:
        lines.append(f"- Energy per atom (Ha): `{record['energy_per_atom_Ha']:.8f}`")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export a markdown analysis report for an ABINIT result directory.")
